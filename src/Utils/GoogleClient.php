...
class GoogleClient {
	...
	public function __call( string $name, $args ) {
		...
		if ( in_array( $name, $methods, true ) && empty( $this->access_token ) ) {
			throw new Exception( __( 'Access token must be set to make this API call', 'rj-google-signin' ) );
		}
	}
	...
	public function gt_redirect_url(): string {
		return apply_filters( 'RjoshiWebdev.google_redirect_url', $this->redirect_uri );
	}
	...
	public function authorization_url(): string {
		...
		$scope = apply_filters_deprecated(
			'wp_google_signin_scopes',
			[
				$plugin_scope,
			],
			'1.0.15',
			'RjoshiWebdev.google_scope'
		);
		 ...
		 */
		$scope = apply_filters( 'RjoshiWebdev.google_scope', $scope );
		 ...
		 */
		$client_args = apply_filters( 'RjoshiWebdev.google_client_args', $client_args );
	...
	public function access_token( string $code ): \stdClass {
		...
		if ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			throw new Exception( __( 'Could not retrieve the access token, please try again.', 'rj-google-signin' ) );
		}
	...
	public function user(): \stdClass {
		try {
			...
			if ( 200 !== wp_remote_retrieve_response_code( $user ) ) {
				throw new Exception( __( 'Could not retrieve the user information, please try again.', 'rj-google-signin' ) );
			}
	...
	public function state(): string {
		...
		$state_data             = apply_filters( 'RjoshiWebdev.google_signin_state', $state_data );
