...
class Authenticator {
	...
	public function authenticate( stdClass $user ): WP_User {
		if ( ! property_exists( $user, 'email' ) ) {
			throw new InvalidArgumentException( __( 'Email needs to be present for the user.', 'rj-google-signin' ) );
		}
		...
		if ( email_exists( $user->email ) ) {
			 ...
			 * @param stdClass $user User data object returned by Google.
			 */
			do_action( 'RjoshiWebdev.google_user_logged_in', $user_wp, $user );
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'RjoshiWebdev.google_register_user', $this->maybe_create_username( $user ) );
	}
	...
	public function register( stdClass $user ): ?WP_User {
		...
		if ( ! $register ) {
			throw new Exception( __( 'Registration is not allowed.', 'rj-google-signin' ) );
		}
		...
		try {
			...
			if ( empty( $whitelisted_domains ) || $this->can_register_with_email( $user->email ) ) {
				 ...
				 */
				do_action( 'RjoshiWebdev.google_user_created', $uid, $user );

				return get_user_by( 'id', $uid );
			}

			/* translators: %s is replaced with email ID of user trying to register */
			throw new Exception( sprintf( __( 'Cannot register with this email: %s', 'rj-google-signin' ), $user->email ) );
